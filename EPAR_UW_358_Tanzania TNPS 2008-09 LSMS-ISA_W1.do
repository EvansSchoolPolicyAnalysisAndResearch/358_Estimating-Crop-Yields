...
global tzn_wave1 "desired filepath/raw data folder name"
...
use "$tzn_wave1/Data_Household_Module_Original/SEC_A_T.dta", clear
keep hhid hh_weight clusterid strataid		
save "$created_data/tzn1_weights_merge.dta", replace


*Gender variables
use "$tzn_wave1/Data_Household_Module_Original/SEC_B_C_D_E1_F_G1_U.dta", clear
ren sbmemno personid		// personid is the roster number, combination of hhid and personid are unique id for this wave
gen female =sbq2==2
gen age = sbq4	
gen head = sbq5==1 if sbq5!=.
keep personid female age hhid head
save "$created_data/tzn1_gender_merge.dta", replace


*Collapsing for gender of head
gen male_head = female==0 & head==1
collapse (max) male_head, by(hhid)
save "$created_data/tzn1_gender_head.dta", replace
...
* LRS *
*******
*First starting with field sizes - long rainy season
use "$tzn_wave1/Data_Agricultural_Module_Original/SEC_2A.dta", clear
*Calculate field area in hectares
gen field_area = area	//GPS measurement
*4,272 generated
replace field_area = s2aq4 if area==0 | area==.			// use farmer's estimate if GPS measurement is 0 or missing
*4,273 changes
replace field_area = field_area*0.404686		// convert acres to hectares


*Status of field
merge 1:1 hhid plotnum using "$tzn_wave1/Data_Agricultural_Module_Original/SEC_3A.dta", gen(_merge_plotdetails_LRS)
*2 not matched
*5,126 matched
gen field_cultivated = s3aq3==1 if s3aq3!=.		// equals one if field was cultivated during LRS

*Gender/age variables
gen personid = s3aq6_1
merge m:1 hhid personid using "$created_data/tzn1_gender_merge.dta", gen(dm1_merge) keep(1 3)		// Dropping unmatched from using
*983 not matched from master 		// 983 missing gender variables
*4,145 matched
tab dm1_merge field_cultivated		// >72% unmatched observations due to field not being cultivated
...
merge m:1 hhid personid using "$created_data/tzn1_gender_merge.dta", gen(dm2_merge) keep(1 3)		// Dropping unmatched from using
*3,379 not matched from master
*1,749 matched
...
merge m:1 hhid personid using "$created_data/tzn1_gender_merge.dta", gen(dm3_merge) keep(1 3)			// Dropping unmatched from using
*5,046 not matched from master
*82 matched
...
keep hhid plotnum field_area field_area_cultivated decision_maker_gender field_cultivated
...
save "$created_data/tzn1_field.dta", replace
...
*Now crops
use "$tzn_wave1/Data_Agricultural_Module_Original/SEC_4A.dta", clear
*Percent of area
gen pure_stand = s4aq3==1
gen any_pure = pure_stand==1			// JDM: pure_stand and any_pure are redundant
...
*Total area on field
bys hhid plotnum: egen total_percent_field = total(percent_field)					// not by zaocode; we want to rescale for entire plot
replace percent_field = percent_field/total_percent_field if total_percent_field>1			// Rescaling; 2,278 changes


*Merging in area from tzn1_field
merge m:1 hhid plotnum using "$created_data/tzn1_field.dta", nogen keep(1 3)
*0 not matched
*5,704 matched

gen crop_area_planted = percent_field*field_area_cultivated

keep crop_area_planted hhid plotnum zaocode dm_* any_* pure_stand decision_maker_gender
save "$created_data/tzn1_crop.dta", replace
...
gen kg_harvest = s4aq15 if s4aq12==1			// Only for those that have finished harvesting
replace kg_harvest = 0 if s4aq2==3
drop if kg_harvest==.							// dropping those with missing kg

keep hhid plotnum zaocode kg_harvest

*Merging area
merge m:1 hhid plotnum zaocode using "$created_data/tzn1_crop.dta", nogen keep(1 3)
*0 not matched
*5,065 matched
...
save "$created_data/tzn1_lrs.dta", replace		// saving BEFORE construction and winsorizing for total yields across both seasons; at the plot-crop level
...
* SRS *
*******
*First starting with field sizes - short rainy season
use "$tzn_wave1/Data_Agricultural_Module_Original/SEC_2B.dta", clear
*Calculate field area in hectares
gen field_area = area	//GPS measurement
replace field_area = s2bq9 if area==0 | area==.			// use farmer's estimate if GPS measurement is 0 or missing 
*74 changes
replace field_area = field_area*0.404686		// convert acres to hectares
*86 changes

*Status of field
merge 1:1 hhid plotnum using "$tzn_wave1/Data_Agricultural_Module_Original/SEC_3B.dta", gen(_merge_plotdetails_SRS) keep(1 3)
*2 not matched
*84 matched (for maize)
gen field_cultivated = s3bq3==1 if s3bq3!=.		// equals one if field was cultivated during SRS

*Gender/age variables
gen personid = s3bq6_1
merge m:1 hhid personid using "$created_data/tzn1_gender_merge.dta", gen(dm1_merge) keep(1 3)		// Dropping unmatched from using
*5 not matched from master 
*81 matched
tab dm1_merge field_cultivated, m		// All unmatched observations due to field not being cultivated or cultivated variable is missing
*First decision-maker variables
gen dm1_female = female
gen dm1_age = age
drop female age personid
...
*Second owner
gen personid = s3bq6_2
merge m:1 hhid personid using "$created_data/tzn1_gender_merge.dta", gen(dm2_merge) keep(1 3)		// Dropping unmatched from using
*35 not matched from master
*51 matched
gen dm2_female = female
gen dm2_age = age
drop female age personid
...
*Third
gen personid = s3bq6_3
merge m:1 hhid personid using "$created_data/tzn1_gender_merge.dta", gen(dm3_merge) keep(1 3)			// Dropping unmatched from using
*80 not matched from master
*6 matched
gen dm3_female = female
gen dm3_age = age
drop female age personid
...
save "$created_data/tzn1_field_srs.dta", replace
...
*Now crops
use "$tzn_wave1/Data_Agricultural_Module_Original/SEC_4B.dta", clear
*Percent of area
gen pure_stand = s4bq3==1
gen any_pure = pure_stand==1
gen any_mixed = pure_stand==0
gen percent_field = 0.25 if s4bq4==1 | s4bq4==.25
...
replace percent_field = percent_field/total_percent_field if total_percent_field>1			// Rescaling; 413 changes


*Merging in area for new srs plots
preserve
merge m:1 hhid plotnum using "$created_data/tzn1_field_srs.dta", nogen keep(3)
*0 not matched
*133 matched
gen lrs_field = 0
tempfile temp
save `temp', replace			// Is the tempfile okay here? It is just for appending five lines below
restore

*Merging in area for srs plots that were also used in lrs
merge m:1 hhid plotnum using "$created_data/tzn1_field.dta", nogen keep(3)
*0 not matched
*1,893 matched
gen lrs_field = 1
append using `temp'
...
keep crop_area_planted hhid plotnum zaocode dm_* any_* lrs_field
save "$created_data/tzn1_crop_srs.dta", replace
...
gen kg_harvest = s4bq15 if s4bq12==1		// Only if finished
...
*Merging area
merge m:1 hhid plotnum zaocode using "$created_data/tzn1_crop_srs.dta", nogen keep(1 3)
*0 not matched
*2,026 matched
...
append using "$created_data/tzn1_lrs.dta", gen(lrs)				// appending plot-crop level data from LRS, collapsing everything to the household level below

*Replacing smaller area with zero (ordering by hhid plotnum zaocode (and crop_area_planted) and replacing first value of crop_area_planted (which should be SMALLER than second) with 0 if second observation is not missing)
bys hhid plotnum zaocode (crop_area_planted): replace crop_area_planted = 0 if _n==1 & crop_area_planted[1]<=crop_area_planted[2] & crop_area_planted[2]!=. // 304 changes
...
save "$created_data/tzn1_field_crop_level.dta", replace


*Collapsing to household level
collapse (sum) kg_harvest* crop_area_planted* count_* (max) dm_*  any_*, by(hhid zaocode)		// collapsing by hhid-crop (zaocode)

*Merging weights and survey variables
merge m:1 hhid using "$created_data/tzn1_weights_merge.dta", nogen keep(1 3)
*0 not matched
*5,245 matched

*Merging Gender of head
merge m:1 hhid using "$created_data/tzn1_gender_head.dta", nogen keep(1 3)
*0 not matched
*5,245 matched

save "$created_data/tzn1_cleaned.dta", replace
...
use "$created_data/tzn1_cleaned.dta", clear
...
gen weight_temp = crop_area_planted*hh_weight
...
replace weight_temp = crop_area_planted_male_dm*hh_weight
svyset clusterid [pweight=weight_temp], strata(strataid) singleunit(centered)
...
replace weight_temp = crop_area_planted_female_dm*hh_weight
svyset clusterid [pweight=weight_temp], strata(strataid) singleunit(centered)
...
replace weight_temp = crop_area_planted_mixed_dm*hh_weight
svyset clusterid [pweight=weight_temp], strata(strataid) singleunit(centered)
...
replace weight_temp = crop_area_planted_purestand*hh_weight
svyset clusterid [pweight=weight_temp], strata(strataid) singleunit(centered)
...
replace weight_temp = crop_area_planted_mixedstand*hh_weight
svyset clusterid [pweight=weight_temp], strata(strataid) singleunit(centered)
...
#delimit ;
	esttab harv1 harv2 harv3 harv4 harv5 harv6 using "$output/tzn_w1_all_`crop_name'.rtf", cells(b(fmt(0)) se(fmt(0) par)) stats(subpop_N, label("Households")) replace
...
gen weight_temp = crop_area_planted_male_pure*hh_weight
svyset clusterid [pweight=weight_temp], strata(strataid) singleunit(centered)
eststo harv1: svy: mean kg_temp kg_MAD_temp
matrix N = e(_N)
estadd scalar subpop_N = N[1,1]
...
replace weight_temp = crop_area_planted_female_pure*hh_weight
svyset clusterid [pweight=weight_temp], strata(strataid) singleunit(centered)
eststo harv2: svy: mean kg_temp kg_MAD_temp
matrix N = e(_N)
estadd scalar subpop_N = N[1,1]
...
replace weight_temp = crop_area_planted_mixed_pure*hh_weight
svyset clusterid [pweight=weight_temp], strata(strataid) singleunit(centered)
eststo harv3: svy: mean kg_temp kg_MAD_temp
matrix N = e(_N)
estadd scalar subpop_N = N[1,1]
...
replace weight_temp = crop_area_planted_male_mixed*hh_weight
svyset clusterid [pweight=weight_temp], strata(strataid) singleunit(centered)
eststo harv4: svy: mean kg_temp kg_MAD_temp
matrix N = e(_N)
estadd scalar subpop_N = N[1,1]
...
replace weight_temp = crop_area_planted_female_mixed*hh_weight
svyset clusterid [pweight=weight_temp], strata(strataid) singleunit(centered)
eststo harv5: svy: mean kg_temp kg_MAD_temp
matrix N = e(_N)
estadd scalar subpop_N = N[1,1]
...
replace weight_temp = crop_area_planted_mixed_mixed*hh_weight
svyset clusterid [pweight=weight_temp], strata(strataid) singleunit(centered)
eststo harv6: svy: mean kg_temp kg_MAD_temp
matrix N = e(_N)
estadd scalar subpop_N = N[1,1]
...
#delimit ;
	esttab harv1 harv2 harv3 harv4 harv5 harv6 using "$output/tzn_w1_gender_`crop_name'.rtf", cells(b(fmt(0)) se(fmt(0) par)) stats(subpop_N, label("Households")) replace
...
use "$created_data/tzn1_lrs.dta", clear
...
*Collapsing to household level
collapse (sum) kg_harvest* crop_area_planted* count_* (max) dm_*  any_*, by(hhid zaocode)				// hhid/crop level
...
*Merging Gender of head
merge m:1 hhid using "$created_data/tzn1_gender_head.dta", nogen keep(1 3)								// 1 not matched from master
keep if inlist(zaocode,`crop')
...
#delimit ;
	esttab harv1 harv2 harv3 harv4 harv5 harv6 using "$output/tzn_w1_LRS_`crop_name'.rtf", cells(b(fmt(0)) se(fmt(0) par)) stats(subpop_N, label("Households")) replace
	mlabels("All" "Male" "Female" "Mixed" "Pure" "Mixed") mgroups("" "Gender of Decision-Maker" "Type of Stand", pattern(1 1 0 0 1 0))
	coeflabels(kg_temp "Yield (kg/ha) - Winsorized" kg_MAD_temp "Yield (kg/ha) - MAD") extracols(2 5)
	;
#delimit cr
...
#delimit ;
	esttab harv1 harv2 harv3 harv4 harv5 harv6 using "$output/tzn_w1_LRS_gender_`crop_name'.rtf", cells(b(fmt(0)) se(fmt(0) par)) stats(subpop_N, label("Households")) replace
	mlabels("Male-Pure" "Female-Pure" "Male-Mixed" "Female-Mixed") coeflabels(kg_temp "Yield (kg/ha) - Winsorized" kg_MAD_temp "Yield (kg/ha) - MAD") extracols(2 5)
	;
#delimit cr
...
*Deleting code to use only largest plots; we are using LRS area for all estimates
duplicates report hhid plotnum zaocode
duplicates report hhid plotnum zaocode lrs			// seems to have worked; there are some duplicates in above line, but none here
bys hhid plotnum zaocode (lrs): replace crop_area_planted=0 if lrs[1]==0 & lrs[2]==1 & _n==1 & crop_area_planted[2]!=0 & crop_area_planted[2]!=.	// replacing crop_area for first obs (srs) if first obs lrs==0 and second obs lrs==1 (but only if crop area from lrs is not zero or missing)
...
save "$created_data/tzn1_field_crop_level_lrsarea.dta", replace
...
save "$created_data/tzn1_cleaned_lrsarea.dta", replace
...
use "$created_data/tzn1_cleaned_lrsarea.dta", clear
keep if inlist(zaocode,`crop')
...
#delimit ;
	esttab harv1 harv2 harv3 harv4 harv5 harv6 using "$output/tzn_w1_LRSarea_all_`crop_name'.rtf", cells(b(fmt(0)) se(fmt(0) par)) stats(subpop_N, label("Households")) replace
	mlabels("All" "Male" "Female" "Mixed" "Pure" "Mixed") mgroups("" "Gender of Decision-Maker" "Type of Stand", pattern(1 1 0 0 1 0))
	coeflabels(kg_temp "Yield (kg/ha) - Winsorized" kg_MAD_temp "Yield (kg/ha) - MAD") extracols(2 5)
	modelwidth(6 2 6 6 6 2 6 6) varwidth(25) note("The decision-maker variable is constructed using the answers to the question Who decided what to plant on this plot in the long rainy season 2008? The decision-maker is coded as male only if all listed decision-makers are male. The variable is coded as female only if all listed decision-makers are female. The variable is otherwise coded as mixed. Winsorized values are replaced at the 99th percentile; any larger values are replaced with the 99th percentile. MAD values are constructed by replacing all values more than two standard deviations above the median with the median. In the top panel statistics include both the long and short rainy seasons, with output aggregated over both seasons but only the largest area planted used as area. In other words, if a household plants one hectare of maize in the long rainy season but two hectares of maize in the short rainy season, two hectares is used as the area for construction of total yield. In the bottom panel, only the long rainy season is included. The decision-maker variables are defined at the plot level but the stand variables are defined as the crop level. As such, some households are represented multiple times across variables. For example, some households have plots under both male-decision makers and female decision-makers, while some other households also have both mixed and pure stand plots. Area weights are used, which are constructed by multiplying the household weight by the area planted. These weights are constructed separately for each subsample.")
	;
...
#delimit ;
	esttab harv1 harv2 harv3 harv4 harv5 harv6 using "$output/tzn_w1_LRSarea_gender_`crop_name'.rtf", cells(b(fmt(0)) se(fmt(0) par)) stats(subpop_N, label("Households")) replace
	mlabels("Male-Pure" "Female-Pure" "Mixed-Pure" "Male-Mixed" "Female-Mixed" "Mixed-Mixed") coeflabels(kg_temp "Yield (kg/ha) - Winsorized" kg_MAD_temp "Yield (kg/ha) - MAD")
	;
#delimit cr
}
